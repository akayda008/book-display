...
- **What was done**, in plain terms:
  1. **Choppy shelf transition, fixed.** The growing-rectangle transition (`src/components/PageTransition.tsx`) was animating the overlay's actual size/position (`x`/`y`/`width`/`height`, which Motion turns into real `top`/`left`/`width`/`height` CSS), forcing the browser to recompute layout on every frame. Rewrote it so the overlay element is always full-viewport-sized and never resized - "growing" and "shrinking" are now done purely with a `transform: translate(...) scale(...)`, animated toward/away from the identity transform, so only `transform`/`opacity` ever animate (this project's own Engineering Principle 2, and the same rule `Book.tsx`'s page-turn already follows). Logged to `docs/MISTAKES.md` since this was a real, already-landed defect, not just a missing feature.
  2. **URL now tracks the current chapter while flipping.** Added an `onChapterChange` callback prop to `src/components/Book.tsx`, fired whenever the on-screen chapter's internal `id` is known (derived from the page id `${chapterId}--page--N`). A new client wrapper, `src/components/ChapterReader.tsx`, uses that callback to call `router.replace(...)` (not `push`) to the current chapter's URL whenever it changes - a shallow, history-only update, so it doesn't add a back-stack entry, doesn't re-run the route's data resolution, and doesn't replay the shelf-opening transition (which only plays on a real navigation). The chapter route (`src/app/[bookSlug]/[chapterSlug]/page.tsx`) now renders `ChapterReader` instead of `Book` directly. This is separate from Sub-project 5's planned `localStorage` reading-position persistence - no `localStorage` logic was added here.
  3. **"Sketchbook Pages" two-page gutter, fixed.** `Book.tsx` had a leftover counter-margin (from Sub-project 3) that specifically canceled the spine-side gutter for image-kind pages, on the reasoning that images don't have a "spine gutter concept." In practice that made the two page slots look inconsistently aligned against each other in two-page mode - empty space sat on the outer edge for images, but the spine side for text. Removed the counter-margin (and the now-unused `renderPageBody` gutter-side parameter) so image pages gutter exactly like text pages.

- **Verified**: Ran `npx tsc --noEmit` (clean), `npm run lint` (clean, zero warnings), and `npm run build` (production build succeeds). Did not launch the app myself - the user's manual pass confirms the visual/behavioral result.

- **What's left**: Nothing scoped to this corrective round is left undone.

## 2026-09-02 — Sub-project 4: Library/shelf view

- **Milestone/cycle**: Sub-project 4 — a library/shelf view with real book/chapter URLs, explicit `slug` fields, and a Motion-based shelf-opening transition.

- **What was done**, in plain terms:
  - Added a `slug` field to `Book` and `Chapter` (`src/types/book.ts`) — a separate, human-readable, hand-set identifier alongside the existing internal `id`, so URLs stay stable even if a title is edited later. Set real slugs on all 15 of `book1`'s chapters, on `book1` itself (`my-work`), and on the former Sub-project 3 test fixture, now a real second book called "Sketchbook Pages" (`sketchbook-pages`).
  - Added a `books` array in `src/data/book.ts` — an actual list the shelf and routing both use, replacing the single hardcoded `book1`. Added `getBookBySlug`/`getChapterBySlug` lookup helpers, and a `validateLibrary` check (runs once, at module load) that prints a console warning if two books or two chapters in the same book share a slug — the same "fail loudly on authoring mistakes" pattern already used for unclosed markdown alignment fences.
  - Built the shelf page (`src/app/page.tsx`): a grid of text-only book cards (title/author/blurb) for every book in the library. Removed the old `?pages=1` temporary query-param switch entirely.
  - Added real routing: `/[bookSlug]/[chapterSlug]` (`src/app/[bookSlug]/[chapterSlug]/page.tsx`) resolves and renders that chapter, or shows the standard 404 page if the book or chapter slug doesn't exist. `/[bookSlug]` (`src/app/[bookSlug]/page.tsx`) redirects to the first chapter for a text book like "My Work"; for a pages-type book like "Sketchbook Pages" (which has no chapters), that same URL renders the reader directly instead of redirecting anywhere.
  - Taught the reader (`src/components/Book.tsx`) to open directly at a specific chapter via a new `initialChapterId` prop — it buffers pages from the book's start (the pagination engine can't skip ahead) until that chapter turns up, then opens there instead of at page 1. The flip animation, pagination engine, and image-page rendering themselves were not touched.
  - Added a generic "card grows into open book" transition (`src/components/PageTransition.tsx`, using the `motion` package per ADR-003): clicking a shelf card grows a plain rectangle from the card's position to fill the screen before the reader appears, and the reader's new "← Back to shelf" link reverses it (shrinks back down while fading) on the way back. Respects `prefers-reduced-motion` (skips the animation, navigates instantly).

- **Decision called out per the brief**: a pages-type book's reader URL is just `/[bookSlug]` with no further segment — since pages-type books have no chapters, there's nothing for a `/[chapterSlug]` segment to mean, so that URL renders the reader directly rather than redirecting anywhere.

- **Verified**: Ran `npx tsc --noEmit` (clean), `npm run lint` (clean, zero warnings), and `npm run build` (production build succeeds — confirms `/`, `/[bookSlug]`, and `/[bookSlug]/[chapterSlug]` all compile as expected route types). Did not launch the app myself — that's the user's manual-test pass.

- **What's left**: Nothing scoped to this milestone is left undone. One known, documented gap carried forward (not a regression, and not something this milestone was asked to fix): a chapter reached via deep-link isn't guaranteed to land on the right-hand page in two-page view, because no part of the pagination engine forces chapters to start on a right-hand page — the same is already true of chapters reached by ordinary forward-flipping. Recorded in `docs/4 - Engineering Context/Architecture.md` under Known Limitations.
